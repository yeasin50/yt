...
When a boy asked you what would you said `Ruti`, then the boy
tell the cooker to prepare Ruti.
...
> When we use the abstract keyword, it refers to an idea, and we don't know who or how it should execute.

After eating you ordered some food to take home.
...
> So we can not simply put int into `String` variable;
...
But our current generic class have one issue. We can wrap the box with a box :) loss of the business owner.
...
We can have function, extensions, classes as generic. You can have more than one generic.
...
You can checkout dart `List`, `Map` dataType or how statefulWidget's state class.
